<template>
  ...
  interface Item {
    ...
    reason: string | null; status: RequestStatus; comments: string | null; created_at: string;
  }
  ...
  const filtered = computed(() => status.value === 'ALL' ? items.value : items.value.filter(i => i.status === status.value));
